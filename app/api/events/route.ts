...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!file) {
            return NextResponse.json({ messsage: 'Image file is required' }, { status: 400 })
        }
        ...
        const uploadResult = await new Promise((resolve, reject) => {
            cloudinary.uploader.upload_stream({ resource_type: 'image', folder: 'EventsApp' }, (error, results) => {
